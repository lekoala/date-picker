...
export class DateCalendarElement extends HTMLElement {
  ...
  getDateState(date) {
    ...
    const disabled = outsideBounds || (explicitlyEnabled ? false : Boolean(state.disabled) || disabledByRule);
  ...
  _onChange(event) {
    ...
    if (target.matches(".dp-month-select")) {
      this._setDisplay(`${this.display.slice(0, 4)}-${target.value}`);
    } else if (target.matches(".dp-year-select")) {
  ...
  _shiftDateMonth(date, amount) {
    const parsed = parseDate(date);
    if (!parsed) return date;
    const target = shiftMonth(date, amount);
    const targetParts = parseDate(`${target}-01`);
    if (!targetParts) return date;
    const nextMonth = shiftMonth(target, 1);
    const last = addDays(`${nextMonth}-01`, -1);
    return `${target}-${String(Math.min(parsed.day, Number(last.slice(8, 10)))).padStart(2, "0")}`;
  }

  /** @param {string} date @param {number} amount */
  _shiftYear(date, amount) {
    const parsed = parseDate(date);
    if (!parsed) return date;
    const year = parsed.year + amount;
    const feb = parsed.month === 2 && parsed.day === 29;
  ...
  render(load = true) {
    ...
    const minYear = this.min ? Number(this.min.slice(0, 4)) : displayYear - 10;
    ...
    for (let year = windowStart; year <= windowEnd; year++) {
      yearOptions += `<option value="${year}"${year === displayYear ? " selected" : ""}>${year}</option>`;
    }
